...
class MFACitems {
    ...
    postDBLoad(container) {
        ...
        //Item Filters
        for (const mmFACID in this.mydb.mmfac_items)
            if (this.mydb.mmfac_items[mmFACID].enable)
                this.addToFilters(mmFACID);
        ...
        for (const weapon in dbMastering) {
            if (dbMastering[weapon].Name == "m1911")
                dbMastering[weapon].Templates.push("weapon_M1911CE_1143x23_MASSIVESOFT");
            if (dbMastering[weapon].Name == "M1A")
                dbMastering[weapon].Templates.push("weapon_M14_762x51_MASSIVESOFT");
        }
        this.logger.debug(modFolderName + " mastery finished");
    }
    cloneItem(itemToClone, mmFACID) {
    ...
    copyToFilters(itemClone, mmFACID, enableCompats = true, enableConflicts = true) {
        ...
        for (const item in this.db.templates.items) {
            if (item in this.mydb.mmfac_items)
                continue;
            ...
            if (enableCompats) {
                for (const filter of filters) {
                    for (const id of filter._props.filters[0].Filter) {
                        if (id === itemClone)
                            filter._props.filters[0].Filter.push(mmFACID);
                    }
                }
            }
            if (enableConflicts)
            ...
            if ("addToThisItemsFilters" in mmfacNewItem) {
                ...
                for (const modSlotName in mmfacNewItem.addToThisItemsFilters) {
                    if (modSlotName === "conflicts")
                        mmFACConflictingItems = mmFACConflictingItems.concat(mmfacNewItem.addToThisItemsFilters.conflicts);
                    else {
    ...
    cloneClothing(articleToClone, mmFACID) {
    ...
    */
    addLocales(mmFACID, mmFACItem, mmfacArticle) {
        ...
        for (const localeID in this.db.locales.global) //For each possible locale/language in SPT's database
         {
            ...
            if (mmfacEntry.locales) {
                ...
                if (modConfig.oldLocales)
                    this.db.locales.global[localeID].templates[mmFACID] = localeEntry;
                //Normal
                else {
                ...
                if (isItem)
                    this.logger.warning("WARNING: Missing locale entry for item: " + mmFACID);
                else
                    this.logger.debug("No locale entries for item/clothing: " + mmFACID);
            }
