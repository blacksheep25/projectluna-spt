...
class WeightlessAmmo {
    ...
    postDBLoad(container) {
        ...
        for (const i in items) {
            ...
            if (parents.includes(items[i]._parent))
                this.changeBullets(items[i]);
        }
        if ((this.config.mag_load_speed_as_percent < 0)) { //First check that user didn't make mag load times a negative number
            ...
            const globals = this.container.resolve("DatabaseServer").getTables().globals.config;
